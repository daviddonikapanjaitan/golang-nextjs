...
func CompleteOrder(c *fiber.Ctx) error {
	...
	go func(order models.Order) {
		...
		ambassadorMessage := []byte(fmt.Sprintf("You earned $%f from link #%s", ambassadorRevenue, order.Code))

		smtp.SendMail("host.docker.internal:1025", nil, "no-reply@email.com", []string{order.AmbassadroEmail}, ambassadorMessage)
